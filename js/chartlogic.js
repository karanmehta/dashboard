...
window.addEventListener("load", () => {
    (async function () {
        const companiesJSON = await getJsonDatafromAPI('/API/companies.json').then(data => {

            data.map((company, index) => {

                getJsonDatafromAPI('/API/performance/countries/company_' + company.id + '.json').then(companydata => {
